...
function textrEnDashes(input) {
  ...
  return input.replace(/((?:\s|^)\d+)-(\d+(?:\s|$))/gim, '$1–$2')
 ...
 * marks are also boldfaced consistently. This comes up often with inline headings and the like.
 */
function normalizeBoldface(input) {
  return input.replace(/\*\*(.*?)\*\*([.:?!])( |$)/gim, '**$1$2**$3')
}
...
export default {
  ...
  standard: (text, cb) => runRemarkPlugins([[remarkTextr, TEXTR_SETTINGS], wrap], text, cb),
  ...
  normalizeFootnotes: (text, cb) => runRemarkPlugins([normalizeFootnotes], text, cb),
  normalizeHeadings: (text, cb) => runRemarkPlugins([normalizeHeadings], text, cb),
