...
function normalizeFootnoteIds(mdast) {
  ...
  atom.notifications.addInfo(`Found footnote ${footnoteRefCount} footnotes and ${newFootnoteIds.size} unique footnote definitions (${Object.keys(footnoteIdMapping).length - newFootnoteIds.size} duplicates dropped)`);
  if (missingFootnoteIds.size) {
    // These are a problem!
    atom.notifications.addWarning(`Found ${missingFootnoteIds.size} footnotes missing definitions: ${Array.from(missingFootnoteIds).sort().join(', ')}`);
  }
