...
function normalizeFootnoteIds(mdast) {
  ...
  atom.notifications.addInfo(`Found ${footnoteRefCount} footnotes and ${newFootnoteIds.size} unique footnote definitions (${Object.keys(footnoteIdMapping).length - newFootnoteIds.size} duplicates dropped)`);
  if (missingFootnoteIds.size) {
    // These are a problem!
    atom.notifications.addWarning(`Found ${missingFootnoteIds.size} footnotes missing definitions (search for .missing): ${Array.from(missingFootnoteIds).sort().join(', ')}`);
  }
  if (orphanedFootnoteIds.size) {
    ...
    atom.notifications.addWarning(`Leaving ${orphanedFootnoteIds.size} orphaned definitions in place (search for .orphan): ${Array.from(orphanedFootnoteIds).sort().join(', ')}`);
  }
}
