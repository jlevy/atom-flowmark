...
export default function attacher(opts) {
  ...
  function visitor(node) {
    ...
    function breakLine(isFormatted) {
      ...
      // Handles Case 3.
      if (isFormatted || currentLine.length > 0) {
        currentLine = [];
    ...
    function process(parent, formatChain) {
      ...
      for (let i = 0; i < parent.children.length; ++i) {
        ...
        if (current.children) {
          ...
          const isFormatted = formatChain.length === 0;
          const words = splitWords(current.value);
          for (let j = 0; j < words.length; ++j) {
            addWord(words[j], j > 0, isFormatted);
          }
