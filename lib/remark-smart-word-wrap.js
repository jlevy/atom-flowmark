...
export default function attacher(opts) {
  ...
  function visitor(node) {
    ...
    // Start accumulating lines fresh from current postion.
    function newText() {
      currentLine = [];
      lines = [currentLine];
    }

    // Break a new line on current text.
    function breakLine() {
      currentLine = [];
    ...
    function process(parent) {
      ...
      for (let i = 0; i < parent.children.length; ++i) {
        ...
        if (current.children) {
          ...
          if (is('text', current)
            && next && position + nodeMinLength(next) + 1 >= width
            && words[words.length - 1].endsWith(' ')) {
            words[words.length - 1] = words[words.length - 1].trimRight();
            breakLine();
          }
